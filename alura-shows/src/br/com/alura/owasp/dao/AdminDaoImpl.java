...
public class AdminDaoImpl implements AdminDao {
	...
	private boolean validaSenhaDoUsuarioComOHAshDoBanco(Usuario usuario, Usuario usuarioRetornado) {
		...
		boolean comparaSenhaComHashESemHash = BCrypt.checkpw(usuario.getSenha(), usuarioRetornado.getSenha());
		return comparaSenhaComHashESemHash;
	}

	private boolean verificaSeUsuarioTemRoleAdmin(Usuario usuarioRetornado) {
		return usuarioRetornado.getRoles().stream().map(Role::getName).collect(Collectors.toList())
				.contains("ROLE_ADMIN");
